...
class IndentPopulator < BasePopulator
  ...
  def indents
    @indents ||= Indent.includes(
      ...
      :forwarded_tos, :requesters, :transaction_froms, :transaction_tos, :forwarded_by, :received_by
    )
  end
  ...
  def filter_by_requested(indents)
    ...
    indents.where(id: current_group.requested_indent_ids)
  end

  def filter_by_received(indents)
    return indents unless received.present? && determine_boolean(received)

    indents.where(id: current_group.forwarded_indent_ids)
  end
